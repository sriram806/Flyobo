...
export const register = async (req, res) => {
  ...
  <div style="font-family: Arial, sans-serif; padding: 24px; background-color: #f9f9f9; color: #333;">
    <div style="max-width: 600px; margin: auto; background: #ffffff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.05); overflow: hidden;">
      <div style="background: #0077B6; color: #fff; padding: 20px 30px;">
        <h1 style="margin: 0;">Welcome to <span style="color: #FFD700;">SWAGATOM TRAVEL PARTNER NETWORK</span>!</h1>
      </div>
      <div style="padding: 30px;">
        <p style="font-size: 18px;">Hi <strong>${name}</strong>,</p>
        <p style="font-size: 16px; line-height: 1.6;">
          We're excited to have your travel agency join the <strong>SWAGATOM</strong> platform! 🌍✈️<br />
          Together, we're going to create unforgettable journeys for students, institutions, and adventurers across the country.
        </p>
        <p style="font-size: 16px; line-height: 1.6;">
          As part of our trusted agency network, you’ll receive exclusive trip requests, event collaboration invites, and timely updates.
        </p>
        <p style="font-size: 16px; line-height: 1.6;">
          Let’s build amazing experiences, one destination at a time!
        </p>
        <p style="margin-top: 30px;">With excitement and wanderlust,<br /><strong>The SWAGATOM Team</strong></p>
      </div>
    ...
    return res.json({ success: true, message: "Registration successful", user: { id: user._id, name: user.name, email: user.email, role: user.role, phone: user.phone } });
  } catch (error) {
